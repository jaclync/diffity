...
3. **Read the PR description** for intent context: `diffity agent description-show`. If it is missing, inaccurate, or out of date relative to the diff, leave a `[suggestion]` general comment about it (the user can then run **/diffity-describe** to fix it — do not rewrite the description yourself during a review).
